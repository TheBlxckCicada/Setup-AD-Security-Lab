...
<#
.SYNOPSIS
    Installs Active Directory Services and sets up a domain called umbrella.corp
...
Write-Output "[+] Installing Windows AD Domain Services and setting up domain"
Write-Output "umbrella.corp created"
...
New-ADOrganizationalUnit -Name "Managers" -Path "DC=umbrella,DC=corp"
NEW-ADGroup –name "Managers" –groupscope Global –path "OU=Managers,DC=umbrella,DC=corp"
New-ADUser -Name "Jack Robinson" -GivenName "Jack" -Surname "Robinson"  -Description "Samrobinson123" -SamAccountName "Jack.Robinson" -UserPrincipalName "J.Robinson@adinsecure.lab" -Path "OU=Managers,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "Samrobinson123" -Force) -Enabled $true
New-ADUser -Name "Joe Standing" -GivenName "Joe" -Surname "Standing"  -Description "Execute Vice Preseident of Strategic Innovation, Holistic Integration and Corporate Development" -SamAccountName "Joe.Standing" -UserPrincipalName "J.Standing@adinsecure.lab" -Path "OU=Managers,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "Executive12399$" -Force) -Enabled $true
Add-ADGroupMember -Identity Managers -Members Jack.Robinson,Joe.Standing
net group "Domain Admins" Joe.Standing /ADD /DOMAIN

New-ADOrganizationalUnit -Name "HR" -Path "DC=umbrella,DC=corp"
NEW-ADGroup –name "HR" –groupscope Global –path "OU=HR,DC=umbrella,DC=corp"
New-ADUser -Name "Katie Haggerty" -GivenName "Katie" -Surname "Haggerty"  -Description "HR Staff1" -SamAccountName "Katie.Haggerty" -UserPrincipalName "K.Haggerty@adinsecure.lab" -Path "OU=HR,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "Str0ngPaSS67" -Force) -Enabled $true
New-ADUser -Name "Olivia Weidman" -GivenName "Olivia" -Surname "Weidman"  -Description "HR Staff1" -SamAccountName "Olivia.Weidman" -UserPrincipalName "O.Weidman@adinsecure.lab" -Path "OU=HR,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "St4SDFxSS11434DF" -Force) -Enabled $true
Add-ADGroupMember -Identity HR -Members Olivia.Weidman,Katie.Haggerty



New-ADOrganizationalUnit -Name "Dev" -Path "DC=umbrella,DC=corp"
NEW-ADGroup –name "Dev" –groupscope Global –path "OU=Dev,DC=umbrella,DC=corp"
New-ADUser -Name "Rob Pratt" -GivenName "Robert" -Surname "Pratt"  -Description ".NET Contractor" -SamAccountName "Robert.Pratt" -UserPrincipalName "R.Pratt@adinsecure.lab" -Path "OU=Dev,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "Password123" -Force) -Enabled $true
New-ADUser -Name "Pritesh Choudwory" -GivenName "Pritesh" -Surname "Choudwory"  -Description ".NET Contractor 2" -SamAccountName "Pritest.Choudwory" -UserPrincipalName "P.Choudwory@adinsecure.lab" -Path "OU=Dev,DC=umbrella,DC=corp" -AccountPassword (Convertto-SecureString -AsPlainText "PaSS12345678" -Force) -Enabled $true
Add-ADGroupMember -Identity Dev -Members Pritest.Choudwory,Robert.Pratt



Write-Output "[+] Creating Kerberoasting setup"
net localgroup administrators UMBRELLA\Robert.Pratt /add
setspn -s http/umbrella.corp:80 Robert.Pratt
...
$Sid = (Get-ADObject -Identity "CN=Robert Pratt,OU=Dev,DC=umbrella,DC=corp").ObjectGUID
...
# Get-ACL "AD:\OU=Managers,DC=umbrella,DC=corp").Access |
# Where-Object {$_.IdentityReference -eq "umbrella\Katie.Haggerty"}  

Write-Output "[+] Creating Passwords in SYSVOL"

new-gpo -name MarketingGPO | new-gplink -target "OU=HR,DC=umbrella,DC=corp" | set-gppermissions -permissionlevel gpoedit -targetname "HR" -targettype group

Write-Output "net user /add Olivia.Weidman St4SDFxSS11434DF" > "C:\Windows\SYSVOL\sysvol\umbrella.corp\scripts\create_backupuser.ps1"
...
"@
$groupspath = "C:\Windows\SYSVOL\sysvol\umbrella.corp\Policies\"
$filepath = Get-ChildItem "C:\Windows\SYSVOL\sysvol\umbrella.corp\Policies\"
...
$objUser = New-Object System.Security.Principal.NTAccount("umbrella.corp", "Joe.Standing") 
